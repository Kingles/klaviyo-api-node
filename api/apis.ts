...
export function queryParamPreProcessor(queryParams: any): void {
  if (
    queryParams["page[cursor]"] &&
    queryParams["page[cursor]"].includes("https://")
  ) {
    const params = new URLSearchParams(
      decodeURI(queryParams["page[cursor]"].split("?")[1])
    );
  ...
  applyToRequest(requestOptions: AxiosRequestConfig): void {
    if (requestOptions && requestOptions.headers) {
      requestOptions.headers[
        "Authorization"
      ] = `${this._apiKeyPrefix} ${this.apiKey}`;
    }
  }
  ...
  applyToRequest(requestOptions: AxiosRequestConfig): void {
    if (requestOptions && requestOptions.headers) {
      if (GlobalApiKeySettings.apiKey) {
        requestOptions.headers[
          "Authorization"
        ] = `${this._apiKeyPrefix} ${GlobalApiKeySettings.apiKey}`;
      } else {
